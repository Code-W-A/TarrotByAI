...
exports.createPaymentIntentAndSave = functions.https.onCall(
    async (data, context) => {
      const {
        amount,
        currency,
      ...
      try {
        console.log("Creare PaymentIntent și Customer în curs...");

        // Creează un client în Stripe
        ...
        console.log("Customer creat cu ID-ul:", customer.id);

        // Creează PaymentIntent asociat cu clientul
        ...
        console.log("PaymentIntent creat cu ID-ul:", transactionId);

        // Creează un InvoiceItem pentru factură
        const invoiceItem = await stripe.invoiceItems.create({
          customer: customer.id,
          amount, // suma facturată
          currency,
          description: `Factura pentru ${firstName} ${lastName}`,
        });

        console.log("InvoiceItem creat cu ID-ul:", invoiceItem.id);

        // Creează și finalizează factura
        const invoice = await stripe.invoices.create({
          customer: customer.id,
          auto_advance: false, // Nu lăsăm Stripe să finalizeze automat
        });

        console.log("Invoice creată cu ID-ul:", invoice.id);

        // Finalizăm și trimitem factura imediat
        ...
        console.log(
            "Invoice finalizată și trimisă cu ID-ul:",
            finalizedInvoice.id,
        );

        // Pregătim documentul pentru Firestore
        const analysisDoc = {
        ...
        // Folosim `add()` pentru a crea un document nou la fiecare plată
        const docRef = await db.collection("analysisBought").add(analysisDoc);

        // Stocăm ID-ul documentului în el însuși
        await docRef.update({documentId: docRef.id});

        console.log("Document Firestore creat cu ID-ul:", docRef.id);

        // Dacă utilizatorul este autentificat, actualizăm telefonul
        if (uid) {
          const userDocRef = db.collection("Users").doc(uid);
          await userDocRef.update({phone: phone});
          console.log("Datele utilizatorului auth au fost actualizate.");
        }
        ...
        return {
          ...
          docId: docRef.id,
          invoiceId: finalizedInvoice.id, // Returnăm ID-ul facturii
          message:
          "PaymentIntent, Customer și Invoice create și finalizate cu succes!",
        };
      } catch (error) {
        console.error(
            "Eroare la crearea PaymentIntent, Customer sau Invoice:",
            error,
        );
        throw new functions.https.HttpsError(
            "internal",
            "Nu s-a putut crea PaymentIntent, Customer sau Invoice.",
        );
      }
    },
);

exports.verifyPaymentIntentStatus = functions.https.onCall(
    async (data, context) => {
      ...
      if (!transactionId) {
        throw new functions.https.HttpsError(
            "invalid-argument",
            "ID-ul tranzacției este necesar.",
        );
      }

      try {
        console.log("Verificăm PaymentIntent cu ID-ul:", transactionId);

        const paymentI = await stripe.paymentIntents.retrieve(transactionId);

        console.log("Status PaymentIntent:", paymentI.status);

        return {status: paymentI.status};
      } catch (error) {
        console.error("Eroare la verificarea PaymentIntent:", error);
        throw new functions.https.HttpsError(
            "internal",
            "Nu s-a putut verifica statusul PaymentIntent.",
        );
      }
    },
);

exports.updateAnalysisIsPaid = functions.https.onCall(async (data, context) => {
  const {docId} = data;

  if (!docId) {
    throw new functions.https.HttpsError(
        "invalid-argument",
        "ID-ul documentului este necesar.",
    );
  }

  try {
    console.log("Actualizăm isPaid la true pentru documentul cu ID-ul:", docId);

    const docRef = db.collection("analysisBought").doc(docId);

    // Actualizăm câmpul isPaid la true
    await docRef.update({isPaid: true});

    console.log("isPaid actualizat cu succes pentru documentul:", docId);

    return {message: "Documentul actualizat cu succes."};
  } catch (error) {
    console.error("Eroare la actualizarea isPaid în Firestore:", error);
    throw new functions.https.HttpsError(
        "internal",
        "Nu s-a putut actualiza documentul din Firestore.",
    );
  }
});

...
exports.checkSendNotAfirmatii = functions.pubsub.schedule("every 5 minutes")
    .timeZone("Europe/Bucharest")
    .onRun(async (context) => {
      const now = new Date();
      const offset = 3;
      ...
      if (articlesSnapshot.empty) {
        console.log("Nicio afirmatie programata pentru acum.");
        return;
      }

      articlesSnapshot.forEach((doc) => article = doc.data());

      console.log("afirmatie primita...", article);
      ...
      for (let i = 0; i < tokens.length; i += BATCH_SIZE) {
        ...
        const messages = batch.map((token) => {
          ...
          console.log("language key from userTokens...", languageKey);
          // console.log("articole info lang...", article.info[languageKey]);
          return {
            to: token.token,
            sound: "default",
            title: article.info[languageKey].nume,
...
exports.checkAndSendNotificationsAfirmatiiIos = functions.pubsub
    .schedule("every 5 minutes")
    .timeZone("Europe/Bucharest")
    .onRun(async (context) => {
      const now = new Date();

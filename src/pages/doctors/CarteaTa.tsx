...
import SingleCardLayout from "../../components/CardLayout/SingleCardLayout";
import SingleFlipCard from "../../components/FlipCard/SingleFlipCard";

const CarteaTa = ({ route }) => {
  ...
  const renderFlipCard = (category, index) => {
    ...
    const getCategoryName = (category, language) => {
      if (language === "hi" && category.info.hu?.nume) {
        return category.info.hu?.nume;
      } else if (language === "id" && category.info.ru?.nume) {
        return category.info.ru?.nume;
      } else if (language === "ru" && category.info.rusa?.nume) {
        return category.info.rusa?.nume;
      } else if (category.info[language]?.nume) {
        return category.info[language]?.nume;
      } else {
        return category.info.en?.nume;
      }
    };
    ...
    const card = shuffledCartiPersonalizate[0];
    const animatedStyle = {
